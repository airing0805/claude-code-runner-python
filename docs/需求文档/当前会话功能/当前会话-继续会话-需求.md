...
| 恢复历史会话 | 从历史记录点击会话 | 加载历史会话到当前界面，可查看并继续执行 |
...
| 会话标题 | 首条用户消息内容（首条 user 消息的 `message.content[0].text`） | 显示在界面标题区域 |
...
> 本节记录 PQ-004 问题的改进需求，详见 [当前会话-主需求.md](当前会话-主需求.md) 评审问题章节。
...
| 需求项 | 说明 |
|--------|------|
| 最近会话获取 API | 调用 `/api/sessions?limit=1&sort_by=timestamp&order=desc` 获取最近会话 |
| 会话 ID 缓存 | 前端缓存最近会话 ID，避免重复请求 |
| 复选框状态同步 | 勾选时自动填充 resume 字段，取消时清空 |
| 空会话处理 | 无历史会话时禁用复选框并提示 |

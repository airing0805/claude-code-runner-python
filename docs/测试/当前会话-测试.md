...
```python
class TestUserQuestionAPI:
    """用户问答 API 测试"""

    def test_answer_endpoint_exists(self):
        """测试问答答案端点存在"""
        from app.routers.task import router
        assert any(route.path == "/answer" for route in router.routes)

    def test_submit_single_choice_answer(self):
        """测试提交单选答案"""
        response = client.post("/api/task/answer", json={
            "session_id": "test-session",
            "question_id": "auth_strategy_01",
            "answer": "oauth2"
        })

        # 如果会话不存在返回 404，会话存在返回 200
...
MOCK_ASK_USER_QUESTION = {
    ...
    "question": {
        ...
        "description": "请选择最适合当前项目安全要求的方案。",
        "type": "multiple_choice",
        "options": [
            {"id": "oauth2", "label": "OAuth 2.0", "description": "支持第三方登录", "default": True},
            {"id": "jwt_local", "label": "JWT + 本地账号", "description": "用户注册/登录使用邮箱密码"},
            {"id": "session_cookie", "label": "Session + Cookie", "description": "服务端维护会话状态"}
        ],
        "required": True
    },

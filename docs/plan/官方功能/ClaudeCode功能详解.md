...
| 响应数据 | 任务统计、费用计算 |
| 内置命令 | /commit、/commit-push-pr、/dedupe、/oncall-triage、/bug |
...
|------|---------|
| `/commit` | 提交当前更改到 Git |
| `/commit-push-pr` | 提交更改、推送到远程并创建 Pull Request |
| `/dedupe` | 去除重复的任务或对话 |
| `/oncall-triage` | 值班分诊，处理生产问题 |
| `/bug` | 报告问题，直接在 Claude Code 内提交 Bug |

### 9.1 使用示例

```bash
# 提交更改
/claude commit -m "feat: 添加新功能"

# 完整工作流
/claude commit-push-pr --title "feat: 添加用户认证" --body "实现了 JWT 认证"

# 报告问题
/claude bug 发现了登录页面的 XSS 漏洞
```

...
添加提问历史记录功能，让用户能够快速查看之前向 Claude 提出的问题，方便重复使用类似的问题。
...
| API | 说明 |
|-----|------|
| `GET /api/projects/{project_name}/questions` | 获取项目提问列表（分页） |
...
- 从会话 JSONL 文件中提取首次用户提问
- 过滤 IDE 标签（`<ide_selection>`, `<ide_opened_file>` 等）
- 敏感信息脱敏（API Keys, GitHub Tokens, Passwords 等）
...
- [提问历史记录-功能介绍.md](提问历史记录-功能介绍.md)
- [v8.0.0-发布说明.md](v8.0.0-发布说明.md)

### Bug 修复

#### 调度器任务取消时的 cancel scope 错误

**问题描述**：
当调度器取消正在执行的任务时，会出现 `RuntimeError: Attempted to exit cancel scope in a different task than it was entered in` 错误。

**根本原因**：
`ClaudeCodeClient.run_stream()` 是 async generator，当外部迭代被取消时（GeneratorExit 异常），async with 上下文管理器的清理代码会在不同的任务中执行，导致 cancel scope 错误。

**修复方案**：
在 `app/claude_runner/client.py` 的 `run_stream` 方法中添加 `GeneratorExit` 异常处理，在取消时正确恢复环境变量。
...
- **服务器启动时自动启动调度器**
  - 在 `app/main.py` 的 `lifespan` 钩子中添加调度器自动启动逻辑
  - 服务器启动后无需手动操作，调度器即可正常运行
  - 启动失败时记录警告日志，不影响服务器运行

#### 代码变更

| 文件 | 变更说明 |
|------|---------|
| `app/main.py` | 导入 `start_scheduler` 函数；在 `lifespan` 中添加自动启动逻辑 |

#### 文档更新

| 文档 | 变更说明 |
|------|---------|
| `docs/技术设计/任务调度-调度器.md` | 新增「自动启动机制」小节，说明服务器启动时的自动启动行为 |
| `docs/技术设计/任务调度-集成设计.md` | 新增集成架构说明和自动启动集成方式 |
| `README.md` | 在快速开始部分添加调度器自动启动说明和启动日志示例 |

#### 行为变化

| 场景 | 之前 | 现在 |
|------|------|------|
| 服务器启动 | 调度器停止状态 | ✅ 调度器自动启动 |
| 服务器重启 | 需要手动启动调度器 | ✅ 调度器自动启动 |
| 定时任务到期 | 需先手动启动调度器 | ✅ 自动检查并执行 |
| 手动添加任务到队列 | 需要手动启动调度器 | ✅ 自动按顺序执行 |
...
完整的任务调度功能，支持任务队列管理、定时任务（Cron）和自动执行。

### 子版本

| 版本 | 发布日期 | 说明 |
|------|---------|------|
| v0.6.0 | 2026-02-26 | 核心调度器与任务队列 |
| v0.6.1 | 2026-02-26 | 定时任务管理 |
| v0.6.2 | 2026-02-27 | 任务执行器与重试机制 |
| v0.6.3 | 2026-02-27 | API 路由实现 |
| v0.6.4 | 2026-02-27 | 前端任务调度视图 |
| v0.6.5 | 2026-02-27 | 前端定时任务管理 |
| v0.6.6 | 2026-02-27 | 前端任务状态展示 |
| v0.6.7 | 2026-02-27 | 测试与优化 |

### 新增功能

#### 任务队列管理
- 添加任务到队列（POST /api/tasks）
- 获取任务队列列表（GET /api/tasks）
- 删除队列中的任务（DELETE /api/tasks/{id}）
- 清空任务队列（DELETE /api/tasks/clear）
...
- `GET /api/scheduler/cron-examples` - 获取常用示例
...
app/routers/
└── scheduler.py      # 调度器 API 路由
```

#### 前端模块
```
web/static/modules/
└── scheduler.js      # 任务调度主模块
```
...
├── running_tasks.json   # 运行中任务
├── completed_tasks.json # 已完成任务
└── failed_tasks.json    # 失败任务
```

### 技术文档
- `docs/技术设计/任务调度-核心设计.md` - 系统架构设计
- `docs/技术设计/任务调度-前端架构.md` - 前端模块设计
- `docs/技术设计/任务调度-调度器状态组件.md` - 状态指示器设计
- `docs/API接口/任务调度-API接口.md` - API 接口文档

...
服务器启动时，通过 `app/main.py` 的 `lifespan` 钩子自动启动调度器：
...
| 组件 | 职责 |
|------|------|
| FastAPI 应用 | 服务器启动时触发调度器启动 |
| 主调度器 | 管理轮询循环、任务调度、定时任务触发 |
| 任务执行器 | 调用 ClaudeCodeClient，管理任务状态和重试 |
| ClaudeCodeClient | 执行具体的任务 prompt |
| 存储层 | 持久化任务状态和历史记录 |

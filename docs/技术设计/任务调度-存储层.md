...
> **重要说明**：当前系统设计为**单实例部署**，所有任务调度在同一个 FastAPI 进程内完成。**不需要文件锁机制**。
> 
> 理由：
> - 单实例部署时不存在并发访问问题
> - 原 Node.js 版本（claude-code-24h-integration）无锁，稳定运行多年
> - 移除锁机制可简化代码，降低复杂度
> - 只有在多实例部署时才需要分布式锁（如 Redis）

### 2.1 文件锁机制（已移除）

> ⚠️ **已废弃**：原设计包含文件锁实现，但经分析单实例部署不需要，现已移除。
> 
> 如需多实例部署，请使用分布式锁（Redis）替代。
...
系统允许创建多个具有相同ID但不同name的定时任务，这会导致：
- `get()` 方法返回错误的任务
- `get_enabled()` 去重逻辑复杂
- `save()` 时任务重复堆积
...
当保存任务时检测到ID重复（但name不同），自动给name添加数字编号后缀：

| 场景 | 处理方式 |
|------|----------|
| ID+name 完全相同 | 更新现有任务 |
| ID 相同，name 不同 | 自动命名为 `原名(n)`，n为递增编号 |
...
def save(self, task: ScheduledTask) -> None:
    ...
    规则：
    - ID+name 相同则更新
    - 如果 ID 相同但 name 不同，自动给 name 添加数字编号后缀
    """
    data = self._read()
    tasks = data.get("tasks", [])
    ...
    found = False
    for i, t in enumerate(tasks):
        if t["id"] == task.id and t.get("name") == task.name:
            tasks[i] = task.to_dict()
            found = True
            break

    if not found:
        # 检查是否有相同ID的任务，如果有则自动编号
        same_id_count = sum(1 for t in tasks if t["id"] == task.id)
        if same_id_count > 0:
            # 找到最大编号
            max_num = 0
            for t in tasks:
                if t["id"] == task.id and t.get("name", "").endswith(")"):
                    import re
                    match = re.search(r"\((\d+)\)$", t.get("name", ""))
                    if match:
                        max_num = max(max_num, int(match.group(1)))
            # 添加编号
            task.name = f"{task.name}({max_num + 1})"
...
```
1. 创建任务: ID=task-1, name="产品总监任务" 
   → 保存为: "产品总监任务"

2. 再次创建: ID=task-1, name="产品总监任务"
   → 更新现有任务

3. 创建任务: ID=task-1, name="产品总监任务(v8)"
   → 保存为: "产品总监任务(v8)(1)"

4. 创建任务: ID=task-1, name="产品总监任务(v9)"
   → 保存为: "产品总监任务(v9)(2)"
```
